...
namespace ICSharpCode.SharpZipLib.Tar
{
	public class TarInputStream : Stream
	{
		...
		public override int Read(byte[] buffer, int offset, int count)
		{
			...
			if (readBuffer != null)
			{
				int num3 = (int)((num2 <= readBuffer.Length) ? num2 : readBuffer.Length);
		...
		public void Skip(long skipCount)
		{
			...
			while (num > 0)
			{
				int count = (int)((num <= array.Length) ? num : array.Length);
			...
			else
			{
				try
				{
					...
					if (tarHeader.TypeFlag == 76)
					{
						...
						while (num > 0)
						{
							int num2 = Read(array2, 0, (int)((num <= array2.Length) ? num : array2.Length));
							if (num2 == -1)
							{
				...
				catch (InvalidHeaderException ex)
				{
					...
					entryOffset = 0L;
					currentEntry = null;
					string message = string.Format("Bad header in record {0} block {1} {2}", buffer.CurrentRecord, buffer.CurrentBlock, ex.Message);
