	...
	public class ZipFile : IDisposable, IEnumerable
	{
		...
		private class UpdateComparer : IComparer
		{
			public int Compare(object x, object y)
			{
				...
				{
					int num2 = ((zipUpdate.Command != UpdateCommand.Copy && zipUpdate.Command != UpdateCommand.Modify) ? 1 : 0);
					int num3 = ((zipUpdate2.Command != UpdateCommand.Copy && zipUpdate2.Command != UpdateCommand.Modify) ? 1 : 0);
					num = num2 - num3;
					if (num == 0)
					{
		...
		public ZipEntry GetEntry(string name)
		{
			...
			return (num < 0) ? null : ((ZipEntry)entries_[num].Clone());
		}
		...
		private long TestLocalHeader(ZipEntry entry, HeaderTest tests)
		{
			lock (baseStream_)
			{
				...
				if (zipExtraData.Find(1))
				{
					if (num < 45)
					{
						throw new ZipException(string.Format("Extra data contains Zip64 information but version {0}.{1} is not high enough", num / 10, num % 10));
					}
		...
		public void CommitUpdate()
		{
			...
			try
			{
				...
				{
					byte[] comment = ((newComment_ == null) ? ZipConstants.ConvertToArray(comment_) : newComment_.RawComment);
		...
		private void ReadEntries()
		{
			...
			for (ulong num18 = 0uL; num18 < num4; num18++)
			{
				...
				{
					zipEntry.CryptoCheckValue = (byte)((num20 >> 8) & 0xFF);
				}
				if (num25 > 0)
				{
