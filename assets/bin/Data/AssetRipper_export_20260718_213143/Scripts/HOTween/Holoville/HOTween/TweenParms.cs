		...
		internal void InitializeObject(Tweener p_tweenObj, object p_target)
		{
			...
			for (int i = 0; i < count; i++)
			{
				...
				else
				{
					...
					string text = ((property != null) ? property.PropertyType.ToString() : fieldInfo.FieldType.ToString());
					switch (text.Substring(text.IndexOf(".") + 1))
					{
					case "Vector2":
					...
					default:
						...
						{
							TweenWarning.Log(string.Concat("No valid plugin for animating \"", p_target, ".", hOTPropData.propName, "\" (of type ", text, "). The tween for this property will not be created."));
							continue;
						}
						break;
					}
