...
namespace NUnit.Framework.Constraints
{
	public class NUnitEqualityComparer : INUnitEqualityComparer
	{
		...
		public bool AreEqual(object x, object y, ref Tolerance tolerance)
		{
			...
			if (FirstImplementsIEquatableOfSecond(type2, type))
			{
		...
		{
			Type[] equatableGenericArguments = GetEquatableGenericArguments(first);
		...
		{
			return Array.ConvertAll(Array.FindAll(type.GetInterfaces(), (Type @interface) => @interface.IsGenericType && @interface.GetGenericTypeDefinition().Equals(typeof(IEquatable<>))), (Type iEquatableInterface) => iEquatableInterface.GetGenericArguments()[0]);
		}

		private static bool InvokeFirstIEquatableEqualsSecond(object first, object second)
		{
			MethodInfo method = typeof(IEquatable<>).MakeGenericType(second.GetType()).GetMethod("Equals");
			return (bool)method.Invoke(first, new object[1] { second });
		}
