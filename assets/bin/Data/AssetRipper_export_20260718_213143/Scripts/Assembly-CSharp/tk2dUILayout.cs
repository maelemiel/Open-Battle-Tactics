...
public class tk2dUILayout : MonoBehaviour
{
	...
	private void Reset()
	{
		...
		if (boxCollider != null)
		{
			...
			Reshape(worldToLocalMatrix.MultiplyPoint(bounds.min) - bMin, worldToLocalMatrix.MultiplyPoint(bounds.max) - bMax, true);
			Vector3 vector = worldToLocalMatrix.MultiplyVector(base.transform.position - position);
			Transform transform = base.transform;
			for (int i = 0; i < transform.childCount; i++)
			{
				Transform child = transform.GetChild(i);
				Vector3 localPosition = child.localPosition - vector;
	...
	public virtual void Reshape(Vector3 dMin, Vector3 dMax, bool updateChildren)
	{
		...
		foreach (tk2dUILayoutItem layoutItem2 in layoutItems)
		{
			...
			{
				layoutItem2.layout.Reshape(vector3, vector4, true);
				continue;
			}
	...
	{
		Matrix4x4 worldToLocalMatrix = base.transform.worldToLocalMatrix;
		Reshape(worldToLocalMatrix.MultiplyPoint(pMin) - bMin, worldToLocalMatrix.MultiplyPoint(pMax) - bMax, true);
	}
