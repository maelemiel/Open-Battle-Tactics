...
public class tk2dUIScrollableArea : MonoBehaviour
{
	...
	[SerializeField]
	private bool allowScrollIfNotFull;
	...
	private float SWIPE_SCROLLING_FIRST_SCROLL_THRESHOLD = 2f;
	...
	private float SCROLL_MARGIN = 0.075f;
	...
	private void Start()
	{
		...
		SWIPE_SCROLLING_FIRST_SCROLL_THRESHOLD = DPI * SCROLL_MARGIN;
	}
	...
	{
		if (allowSwipeScrolling && (allowScrollIfNotFull || contentLength > visibleAreaLength))
		{
			if (!isBackgroundButtonDown && !isSwipeScrollingInProgress)
			{
				tk2dUIManager.Instance.OnInputUpdate += BackgroundOverUpdate;
	...
	private void UpdateSwipeScrollDestintationPosition()
	{
		...
		if (!isSwipeScrollingInProgress && Mathf.Abs(f) > SWIPE_SCROLLING_FIRST_SCROLL_THRESHOLD)
		{
	...
	{
		VisibleAreaLength += ((scrollAxes != Axes.XAxis) ? (dMax.y - dMin.y) : (dMax.x - dMin.x));
	}
	...
	private void ContentLayoutChangeCallback()
	{
		if (contentLayoutContainer != null)
		{
			...
			ContentLength = ((scrollAxes != Axes.XAxis) ? innerSize.y : innerSize.x);
		}
	}
}
