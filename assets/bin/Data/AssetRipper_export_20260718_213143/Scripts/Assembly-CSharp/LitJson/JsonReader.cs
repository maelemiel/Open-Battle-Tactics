...
namespace LitJson
{
	public class JsonReader
	{
		...
		private void ProcessNumber(string number)
		{
			...
			int result2;
			long result3;
			if ((number.IndexOf('.') != -1 || number.IndexOf('e') != -1 || number.IndexOf('E') != -1) && float.TryParse(number, out result))
			{
		...
		public bool Read()
		{
			...
			while (true)
			{
				...
				try
				{
					array = ((current_symbol != 65543 || parse_table[current_symbol].ContainsKey(current_input)) ? parse_table[current_symbol][current_input] : new int[1] { 65550 });
				}
