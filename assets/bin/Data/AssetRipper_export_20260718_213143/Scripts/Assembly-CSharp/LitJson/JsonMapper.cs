...
namespace LitJson
{
	public class JsonMapper
	{
		...
		private static object ReadValue(Type inst_type, JsonReader reader)
		{
			...
			if (reader.Token == JsonToken.Float || reader.Token == JsonToken.Double || reader.Token == JsonToken.Int || reader.Token == JsonToken.Long || reader.Token == JsonToken.String || reader.Token == JsonToken.Boolean)
			{
		...
		private static IJsonWrapper ReadValue(WrapperFactory factory, JsonReader reader)
		{
			...
			{
				jsonWrapper.SetFloat((float)reader.Value);
				return jsonWrapper;
			}
		...
		private static void RegisterBaseImporters()
		{
			...
			importer = (object input) => Convert.ToDouble((float)input);
			RegisterImporter(base_importers_table, typeof(float), typeof(double), importer);
			importer = (object input) => Convert.ToDecimal((float)input);
			RegisterImporter(base_importers_table, typeof(float), typeof(decimal), importer);
