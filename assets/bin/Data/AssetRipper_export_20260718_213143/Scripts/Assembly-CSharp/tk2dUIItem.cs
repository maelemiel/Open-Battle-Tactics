...
public class tk2dUIItem : MonoBehaviour
{
	...
	public string SendMessageOnPreciseClickMethodName = string.Empty;
	...
	private float preciseClickMargin = 0.35f;

	private float preciseClickDots_Sqr;
	...
	public event Action OnPreciseClick;
	...
	public event Action<tk2dUIItem> OnPreciseClickUIItem;
	...
	private void Awake()
	{
		...
		DPI = ((!(Screen.dpi > 0f)) ? 326f : Screen.dpi);
		preciseClickDots_Sqr = DPI * preciseClickMargin * (DPI * preciseClickMargin);
	}
	...
	public bool Press(tk2dUITouch touch, tk2dUIItem sentFromChild)
	{
		...
		{
			this.touch = touch;
			initialPressPosition = touch.position;
			isPreciseClick = true;
	...
	{
		this.touch = touch;
		float sqrMagnitude = (initialPressPosition - touch.position).sqrMagnitude;
		if (sqrMagnitude > preciseClickDots_Sqr)
		{
			isPreciseClick = false;
		}
		if (parentUIItem != null)
		{
	...
	public void Release()
	{
		if (isPressed)
		{
			...
			if (isPreciseClick)
			{
				if (this.OnPreciseClick != null)
				{
					this.OnPreciseClick();
				}
				if (this.OnPreciseClickUIItem != null)
				{
					this.OnPreciseClickUIItem(this);
				}
				DoSendMessage(SendMessageOnPreciseClickMethodName);
				isPreciseClick = false;
			}
		}
