		...
		public static void SpawnPrefabsForChunk(tk2dTileMap tileMap, SpriteChunk chunk, int baseX, int baseY, int layer, int[] prefabCounts)
		{
			...
			for (int i = 0; i < tileMap.partitionSizeY; i++)
			{
				...
				for (int j = 0; j < tileMap.partitionSizeX; j++)
				{
					...
					Object obj = tilePrefabs[tileFromRawTile];
					if (!(obj != null))
					{
						continue;
					}
					...
					if (gameObject == null)
					{
						gameObject = Object.Instantiate(obj, Vector3.zero, Quaternion.identity) as GameObject;
					}
					if (gameObject != null)
					{
						...
						if (!flag)
						{
							gameObject.name = obj.name + " " + prefabCounts[tileFromRawTile];
						}
