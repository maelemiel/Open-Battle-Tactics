...
public class JoinClubController : NewsController
{
	[SerializeField]
	...
	public override IEnumerator Init(NewsDataModel newsDM = null)
	{
		...
		if (newsDM != null)
		{
			...
			{
				_description1.text = newsDM.textDescription3.Localize("- DESCRIPTION 1");
				_description1.Commit();
			}
			if (!string.IsNullOrEmpty(newsDM.textDescription4))
			{
				_description2.text = newsDM.textDescription4.Localize("- DESCRIPTION 2");
				_description2.Commit();
			}
			if (!string.IsNullOrEmpty(newsDM.textDescription5))
			{
				_description3.text = newsDM.textDescription5.Localize("- DESCRIPTION 3");
	...
	{
		Reporting.NewsTemplateClick(NewsTypes.JoinClub, SceneTransitionManager.Scene.HomeScene.ToString(), SceneTransitionManager.Scene.ClubScene.ToString());
