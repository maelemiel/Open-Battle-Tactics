...
namespace Mono.Security.Authenticode
{
	public class AuthenticodeBase
	{
		...
		internal void ReadFirstBlock()
		{
			...
			if (num != 0)
			{
				string text = global::Locale.GetText("Cannot sign non PE files, e.g. .CAB or .MSI files (error {0}).", num);
		...
		internal int ProcessFirstBlock()
		{
			...
			{
				string message = string.Format(global::Locale.GetText("Header size too big (> {0} bytes)."), fileblock.Length);
