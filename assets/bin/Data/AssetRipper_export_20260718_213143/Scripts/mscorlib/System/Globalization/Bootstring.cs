...
namespace System.Globalization
{
	internal class Bootstring
	{
		...
		public string Encode(string s, int offset)
		{
			...
			while (num5 < s.Length)
			{
				...
				foreach (char c in s)
				{
					...
					while (true)
					{
						int num9 = ((num8 <= num3 + tmin) ? tmin : ((num8 < num3 + tmax) ? (num8 - num3) : tmax));
						if (num7 < num9)
						{
							break;
						}
		...
		{
			return (c - 48 < 10) ? (c - 22) : ((c - 65 < 26) ? (c - 65) : ((c - 97 >= 26) ? base_num : (c - 97)));
		}
		...
		public string Decode(string s, int offset)
		{
			...
			while (num5 < s.Length)
			{
				...
				while (true)
				{
					...
					num2 += num9 * num7;
					int num10 = ((num8 <= num3 + tmin) ? tmin : ((num8 < num3 + tmax) ? (num8 - num3) : tmax));
					if (num9 < num10)
					{
						break;
					}
				...
				if (num < 128)
				{
					throw new ArgumentException(string.Format("Invalid Bootstring decode result, at {0}", offset + num5));
				}
