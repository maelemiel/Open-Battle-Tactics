...
namespace System.Security.Cryptography
{
	internal class DESTransform : SymmetricTransform
	{
		...
		private uint CipherFunct(uint r, int n)
		{
			...
			int num2 = n << 3;
			uint num3 = (r >> 1) | (r << 31);
			num |= spBoxes[0 + (((num3 >> 26) ^ array[num2++]) & 0x3F)];
			num |= spBoxes[64 + (((num3 >> 22) ^ array[num2++]) & 0x3F)];
			num |= spBoxes[128 + (((num3 >> 18) ^ array[num2++]) & 0x3F)];
			num |= spBoxes[192 + (((num3 >> 14) ^ array[num2++]) & 0x3F)];
			num |= spBoxes[256 + (((num3 >> 10) ^ array[num2++]) & 0x3F)];
			num |= spBoxes[320 + (((num3 >> 6) ^ array[num2++]) & 0x3F)];
			num |= spBoxes[384 + (((num3 >> 2) ^ array[num2++]) & 0x3F)];
			num3 = (r << 1) | (r >> 31);
			return num | spBoxes[448 + ((num3 ^ array[num2]) & 0x3F)];
		}
