...
namespace Mono.Globalization.Unicode
{
	internal class SortKeyBuffer
	{
		...
		internal void AppendKana(byte category, byte lv1, byte lv2, byte lv3, bool isSmallKana, byte markType, bool isKatakana, bool isHalfWidth)
		{
			...
			AppendBufferPrimitive((byte)((!isSmallKana) ? 228u : 196u), ref l4sb, ref l4s);
			AppendBufferPrimitive(markType, ref l4tb, ref l4t);
			AppendBufferPrimitive((byte)((!isKatakana) ? 228u : 196u), ref l4kb, ref l4k);
			AppendBufferPrimitive((byte)((!isHalfWidth) ? 228u : 196u), ref l4wb, ref l4w);
		}
		...
		public SortKey GetResult()
		{
			...
			int num = l1 + l2 + l3 + l5 + 5;
			int num2 = l4s + l4t + l4k + l4w;
			if (flag)
			{
				num += num2 + 4;
			}
			...
			if (l2 > 0)
			{
				Array.Copy(l2b, 0, array, num3, l2);
			}
			num3 += l2;
			array[num3++] = 1;
			if (l3 > 0)
			{
				Array.Copy(l3b, 0, array, num3, l3);
			}
			num3 += l3;
			array[num3++] = 1;
			if (flag)
			{
				Array.Copy(l4sb, 0, array, num3, l4s);
				num3 += l4s;
				array[num3++] = byte.MaxValue;
				Array.Copy(l4tb, 0, array, num3, l4t);
				num3 += l4t;
				array[num3++] = 2;
				Array.Copy(l4kb, 0, array, num3, l4k);
				num3 += l4k;
				array[num3++] = byte.MaxValue;
				Array.Copy(l4wb, 0, array, num3, l4w);
				num3 += l4w;
				array[num3++] = byte.MaxValue;
			}
			array[num3++] = 1;
			if (l5 > 0)
			{
				Array.Copy(l5b, 0, array, num3, l5);
			}
			num3 += l5;
			array[num3++] = 0;
