...
namespace Mono
{
	internal abstract class DataConverter
	{
		...
		private static DataConverter SwapConv = new SwapConverter();

		private static DataConverter CopyConv = new CopyConverter();
		...
		public static IList Unpack(string description, byte[] buffer, int startIndex)
		{
			...
			while (num2 < description.Length && idx < buffer.Length)
			{
				...
				case 'i':
					...
					{
						arrayList.Add(dataConverter.GetInt32(buffer, idx));
						idx += 4;
					}
					break;
				case 'I':
					if (Prepare(buffer, ref idx, 4, ref align))
					{
						arrayList.Add(dataConverter.GetUInt32(buffer, idx));
						idx += 4;
					}
					break;
				case 's':
					...
					{
						arrayList.Add(dataConverter.GetInt16(buffer, idx));
						idx += 2;
					}
					break;
				case 'S':
					if (Prepare(buffer, ref idx, 2, ref align))
					{
						arrayList.Add(dataConverter.GetUInt16(buffer, idx));
						idx += 2;
					}
					break;
				case 'l':
					...
					{
						arrayList.Add(dataConverter.GetInt64(buffer, idx));
						idx += 8;
					}
					break;
				case 'L':
					if (Prepare(buffer, ref idx, 8, ref align))
					{
						arrayList.Add(dataConverter.GetUInt64(buffer, idx));
						idx += 8;
					}
					break;
				case 'f':
					if (Prepare(buffer, ref idx, 4, ref align))
					{
						arrayList.Add(dataConverter.GetDouble(buffer, idx));
						idx += 4;
					}
					break;
				case 'd':
				...
				case 'z':
				{
					...
					case 1:
						...
						arrayList.Add(encoding.GetChars(buffer, idx, i - idx));
				...
				default:
					throw new ArgumentException(string.Format("invalid format specified `{0}'", description[num2]));
				}
				if (num > 0)
				{
					if (--num > 0)
